...
#----------- sets
set NODES = GRID[0];
set NODES_SWING = setof {i1 in NODES: i1 >= n_lo and i1 < n_td} i1;
#set NODES_SWING = n_lo .. n_td - 1;
set NODES_STANCE = NODES diff NODES_SWING;
set F_N = {i1 in CON_P[1] : i1 mod nc = 0};
set F_T = 1..nc - 1;

#----------- constraints
# state constraints
subject to X0 {i1 in X} : x[i1, 0] = x_0[i1];
subject to XF {i1 in X} : x[i1, GRID_MAX] = x_f[i1];

# simple bounds
subject to XBNDS {i1 in X, i2 in NODES} : x_min[i1] <= x[i1, i2] <= x_max[i1];
#subject to FBNDS {i1 in CON_P[1], i2 in NODES_STANCE} : f_min[i1] <= f[1, i1, i2] <= f_max[i1];
subject to FBNDS {i1 in CON_P[1], i2 in NODES} : f_min[i1] <= f[1, i1, i2] <= f_max[i1];

subject to ABNDS {i1 in Q, i2 in NODES} : -a_lim[i1] <= a[i1, i2] <= a_lim[i1];
subject to UBNDS {i1 in Q, i2 in NODES} : -u_lim[i1] <= u[i1, i2] <= u_lim[i1];

# input constraints
subject to A_RK_LCL {i1 in ODE_SOLVERS, i2 in Q, i3 in ODE[i1, 0], i4 in ODE[i1, 'K']: 1 + ODE_a0[i1] <= i4 <= ODE_K[i1] - ODE_aK[i1]} : a[i2, next(i3,ODE_GRID,i4-ODE_a0[i1])] = a[i2, i3];

subject to F_RK_LCL {i1 in ODE_SOLVERS, i2 in CON_P[1], i3 in ODE[i1, 0], i4 in ODE[i1, 'K']: 1 + ODE_a0[i1] <= i4 <= ODE_K[i1] - ODE_aK[i1]} : f[1, i2, next(i3,ODE_GRID,i4-ODE_a0[i1])] = f[1, i2, i3];

subject to U_RK_LCL {i1 in ODE_SOLVERS, i2 in Q, i3 in ODE[i1, 0], i4 in ODE[i1, 'K']: 1 + ODE_a0[i1] <= i4 <= ODE_K[i1] - ODE_aK[i1]} : u[i2, next(i3,ODE_GRID,i4-ODE_a0[i1])] = u[i2, i3];

subject to NO_FORCE_DURING_JUMPING {i1 in CON_P[1], i2 in NODES_SWING} : f[1, i1, i2] = 0;
	# no contact forces during a jump

# friction constraints
subject to FC_GROUND_1 {i1 in F_N, i2 in F_T, i3 in NODES_STANCE} : -Infinity <= f[1, i1 - i2, i3] - mu_lin * f[1, i1, i3] <= 0;
subject to FC_GROUND_2 {i1 in F_N, i2 in F_T, i3 in NODES_STANCE} : -Infinity <= -f[1, i1 - i2, i3] - mu_lin * f[1, i1, i3] <= 0;
subject to FC_GROUND_3 {i1 in F_N, i2 in NODES_STANCE} : -Infinity <= -f[1, i1, i2] <= 0;
  # these unroll into the resulting friction constraints of reference code

# time constraints
subject to T0 : t[0] = 0;
subject to DT {i1 in ODE_SOLVERS, i2 in ODE[i1, 0]} : dt_min <= ODE_hK[i1, i2] <= dt_max;

#----------- objective
#var cost = sum {i1 in ODE_SOLVERS, i2 in ODE[i1, 0]} ODE_hK[i1, i2] * sum {i3 in ODE[i1, 'K']} (ODE_bK[i1, i3] * (3 * sum {i4 in Q} v[i4, next(i2,ODE_GRID,i3-ODE_a0[i1])]^2) + (0.02 * sum {i4 in CON_P[1]} f[1, i4, next(i2,ODE_GRID,i3-ODE_a0[i1])]^2));
  # integral(3 sum(v^2) + 0.02 sum(f^2))

#minimize JUMP : cost;

minimize JUMP : sum {i1 in GRID[0]} ((3 * sum {i2 in Q} v[i2, i1]^2) + (0.02 * sum {i3 in CON_P[1]} f[1, i3, i1]^2));
  # 3 sum(v^2) + 0.02 sum(f^2)
