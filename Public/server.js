...
<methodCall>
  ...
  if (intMatch) {
    return Number(intMatch[1] || intMatch[2]);
  }
...
app.post("/submit-neos", async (req, res) => {
  try {
    ...
    const solver = extractSolverFromRun(rawRun);

    if (!email) {
      return res.status(400).json({ error: "Email address is required for NEOS submission." });
    }
    ...
    const solverInfo = await findNeosSolverInfo(solver);

    if (!solverInfo) {
      return res.status(400).json({
        error: `Could not find an AMPL-compatible NEOS solver entry for ${solver}.`,
        details: "Check that the solver name and NEOS category match a valid NEOS AMPL solver."
      });
    }

    const category = solverInfo.category;
    const inputMethod = solverInfo.inputMethod;

    console.log("Detected solver:", solver);
    console.log("Selected NEOS category:", category);
    ...
    if (
      ...
      return res.status(500).json({
        ...
        details: job.password || JSON.stringify(job)
      });
    }
...
app.post("/neos-results", async (req, res) => {
  try {
    ...
    const solverOutput = chooseBestOutput({
      resultsTxt,
      jobResults,
      amplOutput,
      jobOut
    ...
    if (jobRecord) {
      ...
      jobRecord.solverOutput = solverOutput;
