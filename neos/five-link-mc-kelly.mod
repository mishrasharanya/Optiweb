...
#-- time constraints
subject to T0: t[0] = 0;
subject to TF: t[GRID_MAX - 1] = step_time_des;

#-- state and input constraints
subject to XT {i1 in X, i2 in GRID[0]} : (if i1 <= nq then -asin(1) else -10) <= x[i1, i2] <= (if i1 <= nq then asin(1) else 10);

subject to U {i1 in Q, i2 in GRID[0]} : if i1 > 1 then -100 <= u[i1, i2] <= if i1 > 1 then 100;

#-- path constraints
subject to STANCE_KNEE_EXTENSION {i1 in GRID[0]} : q[2, i1] >= 0;
subject to SWING_KNEE_EXTENSION {i1 in GRID[0]} : q[5, i1] <= 0;

#-- boundary constraints
# periodicity map: x(T+) - x(0+) = 0
subject to GAITX {i1 in X} : x[i1, GRID_MAX] - x[i1, 0] = 0;

# pre => position at end of step, post => at beginning
subject to SWING_PRE_POSX : g[1, 1, GRID_MAX - 1] - step_length_des = 0;
subject to SWING_PRE_POSY : g[1, 2, GRID_MAX - 1] = 0;

subject to SWING_POST_VELY : Jv[1, 2, 0] >= 0;
subject to SWING_PRE_VELY : Jv[1, 2, GRID_MAX - 1] <= 0;

#-- collision map

subject to QPLUS {i1 in Q} : q[i1, GRID_MAX] - sum {i2 in Q} GAIT_A[i1, i2] * q[i2, GRID_MAX-1] = 0;

#######################
# constraint mapping:    ours       paper
#  right foot (1, 2)     5-/1+ --- 5-/0+    P   1
#  right tib  (3, 4)     4-/2+ --- 4-/1+
#  right fem  (5, 6)     3-/3+ --- 2-/2+
#  torso      (7, 8)     2-/4+ --- 2-/2+
#  left fem   (9, 10)    1-/5+ --- 1-/4+        nq
#  left tib   (11, 12)   0-/6+ --- 0-/5+
#  right tib  (13, 14)   5-/1+ --- 5-/1+    G
#  right fem  (15, 16)   4-/2+ --- 4-/2+
#  torso      (17, 18)   3-/3+ --- 3-/3+
#  left fem   (19, 20)   2-/4+ --- 2-/4+
#  left tib   (21, 22)   1-/5+ --- 1-/5+        nr = 3nq + 2(nq + 1)
#
# our indexing creates the mapping i+ -> (nq - i + 1)-
#######################

set XYTH := 1..3;
set PIDX := 1..nq+1;

var P {i1 in PIDX, i2 in XYTH, i3 in PIM_MBRS: i2 <= 2} = 
  if i3 = '-' then g[1, 2*(i1-1) + i2, GRID_MAX - 1]
  else g[2, 2*(i1-1) + i2, GRID_MAX];

var G {i1 in Q, i2 in XYTH, i3 in PIM_MBRS} = 
  if i3 = '-' then g[1, 2*(nq + 1) + 3*(i1-1) + i2, GRID_MAX - 1]
  else g[2, 2*(nq + 1) + 3*(i1-1) + i2, GRID_MAX];

var Gdot {i1 in Q, i2 in XYTH, i3 in PIM_MBRS} = 
  if i3 = '-' then Jv[1, 2*(nq + 1) + 3*(i1-1) + i2, GRID_MAX - 1]
  else Jv[2, 2*(nq + 1) + 3*(i1-1) + i2, GRID_MAX];

var preimpact_ang_mom {i1 in Q} =
  sum {i2 in Q: i2 <= i1} (
    robot[i2, 'MASS'] * (
      (G[nq - i2 + 1, 1, '-'] - P[nq - i1 + 1, 1, '-']) * Gdot[nq - i2 + 1, 2, '-']
      - (G[nq - i2 + 1, 2, '-'] - P[nq - i1 + 1, 2, '-']) * Gdot[nq - i2 + 1, 1, '-']
    )
    + robot[i2, 'IZZ'] * Gdot[nq - i2 + 1, 3, '-']
  );

var postimpact_ang_mom {i1 in Q} =
  sum {i2 in Q: i2 >= i1} (
    robot[i2, 'MASS'] * (
      (G[nq - i2 + 1, 1, '+'] - P[nq - i1 + 2, 1, '+']) * Gdot[nq - i2 + 1, 2, '+']
      - (G[nq - i2 + 1, 2, '+'] - P[nq - i1 + 2, 2, '+']) * Gdot[nq - i2 + 1, 1, '+']
    )
    + robot[i2, 'IZZ'] * Gdot[nq - i2 + 1, 3, '+']
  );

subject to CONSV_ANG_MOM {i1 in Q} : preimpact_ang_mom[nq - i1 + 1] = postimpact_ang_mom[i1];
...
param refa {Q, GRID[0]};
param refu {Q, GRID[0]};
param refB {Q, Q}; # coordinate converter
#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#*#* gait!
